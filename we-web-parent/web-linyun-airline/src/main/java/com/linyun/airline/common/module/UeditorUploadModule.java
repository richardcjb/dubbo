package com.linyun.airline.common.module;
...
public class UeditorUploadModule {
	...
	@At
	@Ok("json")
	public String imageUpload(HttpServletRequest request, HttpServletResponse response, @Param("action") String action)
			throws Exception {
		...
		if (action.equals("uploadimage")) {
			...
			if (null != url) {
				result = "{\"state\": \"SUCCESS\",\"title\": \"\",\"original\":\"" + origFileName + "\",\"type\": \""
						+ uploader.getType() + "\",\"url\":\"" + CommonConstants.IMAGES_SERVER_ADDR + url
		...
		//2复制黏贴多张图片
			...
			for (int i = 0; i < source.length; i++) {
				try {
					...
					String newUrl = fdfsUploadService.uploadImage(uc.getInputStream(), fileExt, null);

					if (i == source.length - 1) {

						buffer.append("{\"state\": \"SUCCESS\",\"title\": \"\",\"source\":\"" + source[i]
								+ "\",\"type\": \"" + fileExt + "\",\"url\":\"" + CommonConstants.IMAGES_SERVER_ADDR
								+ newUrl + "\",\"size\": \"" + uc.getContentLength() + "\"}]}");
					} else {
						buffer.append("{\"state\": \"SUCCESS\",\"title\": \"\",\"source\":\"" + source[i]
								+ "\",\"type\": \"" + fileExt + "\",\"url\":\"" + CommonConstants.IMAGES_SERVER_ADDR
								+ newUrl + "\",\"size\": \"" + uc.getContentLength() + "\"},");
